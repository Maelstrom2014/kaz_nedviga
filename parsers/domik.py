...
class DomikParser(BaseParser):
    """domik.kz — портал недвижимости."""
    name = "domik.kz"
    base_url = "https://domik.kz"
    ...
    def _parse_soup(self, soup: BeautifulSoup, params: SearchParams) -> list[Listing]:
        ...
        for card in soup.select("div.ser-item, div.list-item, .object-item, div.catalog-item"):
            a = card.select_one("a.ser-item__title, a.title, h3 a, a[href*='/arenda/']")
            if not a:
                continue
            ...
            loc = card.select_one(".ser-item__address, .address, .location")
    ...
    def _extract_price(card) -> int | None:
        el = card.select_one(".ser-item__price, .price, .object-price")
