...
class AllrealtyParser(BaseParser):
    """allrealty.kz — портал недвижимости."""
    name = "allrealty.kz"
    base_url = "https://allrealty.kz"
    search_path = "/arenda-kvartir/almaty"
    ...
    def _parse_soup(self, soup: BeautifulSoup, params: SearchParams) -> list[Listing]:
        ...
        for card in soup.select("div.catalog-item, div.card-item, .object-card, div.property-card"):
            a = card.select_one("a.catalog-item__title, a.title, h3 a, a[href*='/object/']")
            if not a:
                continue
            ...
            loc = card.select_one(".catalog-item__address, .address, .location")
    ...
    def _extract_price(card) -> int | None:
        el = card.select_one(".catalog-item__price, .price, .object-price")
